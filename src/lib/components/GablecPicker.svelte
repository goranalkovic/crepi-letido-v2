			...
			<AlertDialog.Content>
				<AlertDialog.Header>
					...
					<AlertDialog.Description
						...
						/>Žalbe se mogu poslati na <code class="font-mono">uprava@vodo-vod.ck.mdmjr</code></AlertDialog.Description
					>
				</AlertDialog.Header>
				<AlertDialog.Footer>
				...
				<Card.Content class="w-full pt-6">
					<div class="flex flex-col gap-1">
						{#each meals as { name, price, meta, isCustomItem, hasCustomText, hasTopSeparator }, index (index + 1)}
							...
							<div class="flex items-center {$mealSelectionData?.currentUserData?.selected?.[restaurant?.slug]?.find(({ meal }) => meal === index) ? 'gap-0.5' : 'gap-1'}">
								<Toggle
									class="justify-start p-4 w-full {hasCustomText || isCustomItem ? 'h-12' : 'h-auto'} {(hasCustomText || isCustomItem) && $mealSelectionData?.currentUserData?.selected?.[restaurant?.slug]?.find(({ meal }) => meal === index) ? 'rounded-r-none' : ''}"
								...
								{#if isCustomItem && restaurant?.url}
									<Button href={restaurant?.url} target="_blank" rel="noopener noreferrer" variant={$mealSelectionData?.currentUserData?.selected?.[restaurant?.slug]?.find(({ meal }) => meal === index) ? 'secondary' : 'ghost'} class="h-12 {$mealSelectionData?.currentUserData?.selected?.[restaurant?.slug]?.find(({ meal }) => meal === index) ? 'rounded-none' : ''} transition">
...
{:else if $resturantData?.length > 0 && $mealSelectionData?.currentUserData?.final}
	<div class="flex flex-col items-center justify-center py-20">
		...
		<Button class="mt-12" on:click={() => $visibleTab = 'overview'}>Pogleč kaj se izabralo</Button>
	</div>
{:else}

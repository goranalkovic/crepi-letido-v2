...
{:else if $resturantData?.length > 0 && !$mealSelectionData?.currentUserData?.final}
	...
	<div class="flex flex-wrap justify-center gap-8">
		{#each $resturantData as { meals, restaurant, id } (id)}
			<section
				class="relative overflow-hidden border w-96 rounded-xl border-border shrink-0"
			>
				<div class="grid grid-cols-1 grid-rows-1 overflow-hidden h-44">
					<img
						class="w-48 h-32 col-start-1 col-end-1 row-start-1 row-end-1 duration-1000 scale-150 translate-x-8 translate-y-2"
					...
					>
						<div
							class="flex items-center self-center justify-center w-12 border rounded shadow-sm aspect-square bg-background/90 border-muted"
						>
							<img
				...
				<div
					class="flex flex-col gap-1 py-4 overflow-y-auto border-t h-72 border-border scrollbar-hide"
				>
					{#each meals as { name, price, meta, isCustomItem, hasCustomText, hasTopSeparator }, index (index + 1)}
						...
						<div
							class="flex items-center mx-4 {userSelections?.[
								restaurant?.slug
						...
						>
							<Toggle
								...
								isCustomItem
									...
									isCustomItem) &&
								userSelections?.[restaurant?.slug]?.find(
									({ meal }) => meal === index,
								)
									? 'rounded-r-none'
									: ''}"
								pressed={userSelections?.[
									restaurant?.slug
								]?.find(({ meal }) => meal === index)}
								onPressedChange={(v) =>
									handleMealSelect(
							...
							{#if isCustomItem && restaurant?.url}
								<Button
									...
									variant={userSelections?.[
										restaurant?.slug
									]?.find(({ meal }) => meal === index)
										? "secondary"
										: "ghost"}
									class="h-12 {userSelections?.[
										restaurant?.slug
									]?.find(({ meal }) => meal === index)
										? 'rounded-none'
										: ''} transition"
							...
							{#if hasCustomText && userSelections?.[restaurant?.slug]?.find(({ meal }) => meal === index)}
								<Dialog.Root
									onOpenChange={(open) => {
										if (!open) {
								...
								>
									...
									<Dialog.Content>
										<Dialog.Header>
											...
											>
											<Dialog.Description
												>Ak imaš neku želju za nekaj od
												jela, napiši ovdje.</Dialog.Description
											>
										</Dialog.Header>

										<Textarea
											bind:value={tempNoteText}
											id="mealNotes-r-{id}-m-{index + 1}"
										/>
				...
				<div
					class="absolute bottom-0 left-0 right-0 z-10 h-6 pointer-events-none bg-gradient-to-t from-background to-background/0"
				></div>
				<div
					class="absolute left-0 right-0 z-10 h-3.5 mt-px pointer-events-none top-44 bg-gradient-to-b from-background to-background/0"
				></div>
			</section>
		{/each}
	</div>

<script>
	...
	const getGablecData = async () => {
		...
		return allRestaurantElements
			.map((restaurant) => {
				const titleRaw = restaurant.querySelector(
					".main-restoran-title",
				);
				...
				let output = {
					...
					meals: mealsRaw
						...
						?.map((mealData) => {
							...
							const isVegetarian = mealData?.classList?.contains(
								"food-type--vegetarijansko",
							);
							const isVegan = mealData?.classList?.contains(
								"food-type--vegetarijansko",
							);

							return {
								name: mealData
									.querySelector(".naziv-jela")
									?.innerText?.trim(),
	...
	const mealSelectionData = readable(null, (set) => {
				...
				.then(({ data }) => {
					if (!data) {
						supabase
							...
							.select()
							.then(({ data: currentUserData }) =>
								set({ currentUserData, allSelectionData }),
							);
					} else {
		...
		const channel = supabase
			...
			.on(
				"postgres_changes",
				{ event: "*", schema: "public", table: "meal-selections" },
				({ eventType, new: newRecord }) => {
	...
	const resturantData = readable(null, (set) => {
		...
		const channel = supabase
			...
			.on(
				"postgres_changes",
				{ event: "*", schema: "public", table: "meal-data" },
	...
	const processUserSelections = async () => {
		...
		const selectionData = rawRestaurantData.reduce(
			(prev, { restaurant: { slug }, meals }) => {
				if (!meals) {
					return prev;
				}
		...
		const intersects = Object.entries(selectionData)
...
<div class="p-10 pt-32">
	<div class="flex items-center gap-4 p-2 rounded border border-muted">
		<Button
			...
			href="/gableci/pick">Pick-a-gablec</Button
		>
		<Button disabled={$page.route.id === "/gableci"} variant={$page.route.id === "/gableci" ? 'default' : 'outline'} href="/gableci"
			>Svi odabiri</Button
		>
	</div>

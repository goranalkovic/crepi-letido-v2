<script>
	...
	const processUserSelections = async () => {
		...
		const goldenIntersects = Object.entries(refinedUserSelections).filter(([_, selections]) => selections.length > 0)
		...
		const intersects = Object.entries(refinedUserSelections).filter(([_, selections]) => selections.length > 0)
			.map(([restName, selections]) => {
...
<div class="p-10 pt-32">
	<div class="flex items-center gap-px mx-auto mb-4 w-fit">
		<Button
			...
			href="/gableci/pick"
			class="rounded-r-none">Pick-a-gablec</Button
		>
		<Button
			disabled={$page.route.id === "/gableci"}
			variant={$page.route.id === "/gableci" ? "default" : "outline"}
			class="rounded-l-none"
			href="/gableci">Svi odabiri</Button
		>
	</div>
